// attach-only overlay; draggable/resizable; per-symbol memory; styled lines + badges
(() => {
  let mounted=false, layer=null, panel=null, priceMap=null, cur="SYM", perSymbol=true;

  function findRoot(){
    const c=[...document.querySelectorAll("div,section,main")].filter(el=>{
  ...
  function mapYAxis(root){
    ...
    const pts=labels.slice(0,10).map(el=>({
      ...
      p:parseFloat(el.textContent.replace(/[^0-9.]/g,''))||0
    }));
  ...
  function savePos(){
    if(!panel) return;
    const key=`tt_ui_${cur}`;
    const d={ left:panel.style.left, top:panel.style.top, scale:panel.dataset.scale||"1" };
  ...
  function ensure(){
    ...
    Object.assign(panel.style,{position:"absolute",top:"8px",left:"8px",pointerEvents:"auto",display:"flex",
      alignItems:"center",gap:"6px",background:"rgba(10,13,22,.92)",color:"#eaeef5",border:"1px solid #223",
      borderRadius:"10px",padding:"6px 8px",font:"12px system-ui,Segoe UI,Arial",zIndex:2147483601});
    panel.innerHTML = `
    ...
    panel.addEventListener("click", e=>{
      ...
      if(cmd==="scale"){ const curS=Number(panel.dataset.scale||"1"); const nxt=curS===0.7?0.9:(curS===0.9?0.8:0.7); panel.dataset.scale=nxt; panel.style.transform=`scale(${nxt})`; panel.style.transformOrigin="top left"; savePos(); }
    ...
    // Re-anchor on DOM changes
    const mo = new MutationObserver(()=>{ priceMap=mapYAxis(findRoot()); });
    mo.observe(document.documentElement,{childList:true,subtree:true,attributes:true});
    window.addEventListener("resize", ()=> priceMap=mapYAxis(findRoot()));
    window.addEventListener("scroll", ()=> priceMap=mapYAxis(findRoot()));
  }
  ...
  chrome.runtime.onMessage.addListener((msg,_,send)=>{
    if (msg?.type==="TT_ATTACH"){ mounted=true; cur=msg.symbol||"SYM"; ensure(); send&&send({ok:true}); }
